...
$$ e^{\lambda |x|} \leq e^{\lambda x} + e^{-\lambda x} $$ for any $$ x $$
\begin{align}
  \P (| X - \E [X] | \geq t)
  &\leq
  \inf\_{\lambda > 0}
  ...
  \, .
\end{align}
Our approach is based on symmetrisation. 
Introduce $$ Y $$ an independent copy of $$ X $$ and use
...
\begin{align}
  ...
  \, ,
\end{align}
and assuming integrability (which we prove momentarily) rewrite
\begin{align}
  \E [e^{\lambda (X - Y)}]
  =
  1
  ...
  \, .
\end{align}
Using that $$ X - Y $$ is symmetric, it is clear that every finite _odd_ moment
vanishes, which allows us to restrict our attention to bounding the even
moments. This can be done as before via the layer cake trick
\begin{align}
  \E [|X - Y|^n]
  &=
  \int\_0^\infty
...
\begin{align}
  ...
  \, ,
\end{align}
where (i) uses the [closed-form expression](https://en.wikipedia.org/wiki/Normal_distribution#Moments)
...
Substituting the bound for the even moments of our MGF bound
\begin{align}
  \E [e^{\lambda(X - Y)}]
  &=
  1
...
(iii) exploits $$ c_3 \geq 1 $$: since the bound $$ P(|X - m_X| \geq t) \leq c_3 e^{-c_4 t^2} $$ holds for all $$ t \geq 0 $$, taking $$ t \downarrow 0 $$ reveals $$ c_3 \geq 1 $$.

Since the same argument can be applied to bound
...
<span class="accent">
   This bound is valid but still **does not match the required constants!**
</span>
...
Introducing $$ Y $$ as an independent copy of $$ X $$, we can use the convexity of $$ x \mapsto e^{\lambda x^2} $$
