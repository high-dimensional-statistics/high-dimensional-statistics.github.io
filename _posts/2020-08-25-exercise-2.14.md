...
Let us with the standard Chernoff bound
\begin{align}
  \P (| X - \E [X] | \geq t)
  \leq
  2 \P (X - \E [X] \geq t)
...
\begin{align}
  \E [|Z|^{n - 1}]
  =
  \bigl( \tfrac{2}{c\_4} \bigr)^{\frac{n - 1}{2}}
  \frac{
...
\begin{align}
  ...
  \newline
  &\leq
  (c\_3 \vee 1) e^{\frac{4 \lambda^2}{c\_4}}
  \, ,
\end{align}
where (i) uses the identity $$ \Gamma (2k) = (2k - 1)! $$, and (ii) follows by
the same identity and $$ \frac{1}{(2k - 1) \cdots k} \leq \frac{1}{k!} $$.

Substituting into our Chernoff bound
\begin{align}
  \P (|X - \E [X]| \geq t)
  &\leq
  2 (c\_3 \vee 1) \inf_{\lambda > 0}
    e^{\frac{4 \lambda^2}{c\_4} - \lambda t}
  =
  c\_1 e^{- c\_2 t^2}
  \, ,
\end{align}
with $$ c_1 = 2 (c_3 \vee 1) $$ and $$ c_2 = \frac{c_4}{8} $$.


<span class="accent">
   As before, this bound is valid but **does not match the required constants!**
   While $$ c_3 $$ is usually larger than $$ 1 $$ and so the prefactor is
   essentially what was required, the constant in the exponent is still worse by
   a factor of two.
</span>
