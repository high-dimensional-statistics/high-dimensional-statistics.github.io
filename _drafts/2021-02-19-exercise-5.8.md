...
**Sketch proof:** Any $$\varepsilon$$-packing $$M$$ of
...
with the balls defined w.r.t. the Hamming metric. As an aside, if we were to go
down the vanilla Gilbert-Varshamov way, we would now upper bound the r.h.s. by
$$| M | \max_{m \in M} |B_{\varepsilon} (m)|$$, obtaining a lower bound for
$$|M|$$.
...
least twice that of $$\lbrace 0, 1\rbrace_{s_0}^d$$ since for any $$m \in M$$,
we can embed $$\theta_{i} = \tfrac{m_i}{\sqrt{s_0}}$$ and
...
Choosing $$s_0 = \lceil \tfrac{s}{2} \rceil$$, and using the change of logarithm
base formula, we obtain
